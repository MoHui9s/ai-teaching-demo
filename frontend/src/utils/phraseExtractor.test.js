...
describe('wrapEnglishPhrases', () => {
  ...
  it('emoji 作为边界不包裹在英语中', () => {
    ...
    const result = wrapEnglishPhrases(input)
    // emoji 和中文字段不应在 span 内
    expect(result).toContain('📢')
    expect(result).toContain('Tom:') // 注意这里的 : 可能把 Tom: 也包裹了...
    // 实际上 "Tom: Hello, welcome to my shop!" 整个是英文字符序列
...
// ──────────────────────────────────────
//  extractEnglishPhrase （保留以验证向后兼容）
// ──────────────────────────────────────
...
describe('extractEnglishPhrase', () => {
  ...
  it('从点击文本节点中提取英文短语', () => {
    ...
    const phrase = extractEnglishPhrase(el, textNode, 6) // "world" 的 w
